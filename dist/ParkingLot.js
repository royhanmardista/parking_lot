 ...
 */
class ParkingLot {
    constructor(maxSpace, feePerHour) {
        this.parkingTickets = [];
     ...
     */
    parkCar(carNumber) {
        if (this.maxSpace >= this.parkingTickets.length) {
    ...
    leavePark(carNumber, timeSpent) {
        ...
        if (foundTicketIndex !== -1) {
            ...
            console.log(`Registration number ${this.parkingTickets[foundTicketIndex].carNumber} 
                   with Slot Number ${foundTicketIndex + 1} is free with Charge ${cost}`);
